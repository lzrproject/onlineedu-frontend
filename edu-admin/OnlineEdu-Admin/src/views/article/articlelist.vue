<template>
    <div class="app-container">
        <div class="search-box">
            <el-form ref="searchParamsForm" :inline="true" :model="searchParams" size="small">
              <el-form-item label="文章名：" prop="activityName">
                 <el-input v-model="searchParams.username" placeholder="输入文章名搜索" />
              </el-form-item>
      ...
      <el-button type="primary" size="small" icon="el-icon-plus" @click="createDialogVisible=true">创建活动</el-button>
    </div>
    <div class="blog-list">
      <el-table
        ref="listTable"
    ...
    <div>
      ...
      >
        ...
        <span slot="footer">
          <el-button type="primary" size="small" @click="createActivity">创 建</el-button>
...
import {listAll,getInfo,deleteArticle,createArticle,listuserArticle,disable,enable} from '@/api/article'

export default {
  name: 'ActivityList',  
  components: {  
    'v-article-add-update': () => import('@/views/article/article_add_update')  
  },  
  data() {  
    return {  
      searchParams: {  
      ...
      // 帖子标题  
      activityTitleList: ['帖子 1', '帖子 2', '帖子 3'],
      // 启用、封禁
  ...
  methods: {
    ...
    openEnableDialogVisible(index) {
      ...
      this.enableDialogTitle = `${activity.enable ? '启用' : '封禁'}活动[${activity.activityName}]`
    ...
    disenablle() {
      ...
      fun(activity.id).then(resp => {
        const msg = '活动[' + activity.activityName + ']已' + (activity.enable ? '启用' : '禁用')
        if (activity.enable) {
